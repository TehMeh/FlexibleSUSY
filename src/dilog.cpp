...
precise_real_type dilog(precise_real_type x) noexcept {
   ...
   const precise_real_type HF  = 0.5;
   ...
   const precise_real_type C[20] = {0.42996693560813697, 0.40975987533077105,
     -0.01858843665014592, 0.00145751084062268,-0.00014304184442340,
      0.00001588415541880,-0.00000190784959387, 0.00000024195180854,
     -0.00000003193341274, 0.00000000434545063,-0.00000000060578480,
      0.00000000008612098,-0.00000000001244332, 0.00000000000182256,
...
{
   const precise_real_type PI = 3.141592653589793238l;
   ...
   if (iz == 0.l) {
      if (rz <= 1.l)
         return precise_complex_type(dilog(rz), 0.l);
      if (rz > 1.l)
   ...
   if (rz <= 0.5l) {
      if (az > 1.l) {
         cy = -0.5l * sqr(precise_complex_type(log(-z)));
         cz = -log(1.l - 1.l / z);
         jsgn = -1;
         ipi12 = -2;
      } else { // (az <= 1.)
   ...
   return static_cast<precise_real_type>(jsgn) * sum + cy + ipi12 * PI * PI / 12.l;
} // S.D.
